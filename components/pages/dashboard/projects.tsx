    ...
    return (
        <div className='d-flex flex-column justify-content-center align-items-center' style={{ background: 'url(/img/projects-bg.jpg)', width: '100%', backgroundRepeat:'none', backgroundSize:'cover'}}>
            ...
            <div className='d-flex flex-md-row flex-column row justify-content-md-evenly align-items-center mb-5' style={{width:'100%'}}>
                {projects.map((el) => (
                    <div className='col-md-3 col-9 d-flex flex-column justify-content-center align-items-center rounded px-0 mb-md-0 mb-4' style={{ background: '#fff'}}>
                        <div style={{ width: '100%' }}>
                            <img src={`/img/${el.img}`} alt="" style={{ width: "100%" }} className='rounded' />
                        </div>
                        <span className='mt-2 linkVisit cursor-pointer' style={{ borderRadius: '100px', padding: '5px 0px', width: '30%', textAlign: 'center', border: '2px solid #DDB146' }}><a href={`${el.link1}`} className='linkVisit' target={'_blank'}>Visit</a></span>
                        <span className='mt-2 linkVisit cursor-pointer mb-2' style={{ borderRadius: '100px', padding: '5px 0px', width: '30%', textAlign: 'center', border: '2px solid #DDB146' }}><a href={`${el.link2}`} className='linkVisit' target={'_blank'}>Code</a></span>
                    </div>
                ))}
            </div>
        </div>
